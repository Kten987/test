...
class RealEstatePredictor:
    def __init__(self):
        self.model_filename = r'RandomForestRegressor_rental_cost.pkl'
        self.image_filename = r'OK2.jpg'
        self.data_filename = r'all_bds_final_clean.csv'
    ...
    def run(self):
        ...
        df_median = df.groupby(['City', 'District'])['price_per_m2_final'].median().reset_index()
        ...
        df_median2 = df.groupby(['City','District','Ward'])['price_per_m2_final'].median().reset_index()
        df_wardd = df_median2[df_median2['District'] == selected_district]
        fig = px.bar(df_wardd, x='Ward', y='price_per_m2_final', title=f"Biểu đồ giá/m2 theo Phường - Quận {selected_district} - TP {selected_city}")
        fig.update_layout(xaxis_title='Phường', yaxis_title='Trung vị giá / m2', xaxis_tickangle=-45, width=1200, height=700)
        st.plotly_chart(fig)

        df_median3 = df.groupby(['City','District','Ward','Street'])['price_per_m2_final'].median().reset_index()
        df_street = df_median3[df_median3['Street'] == selected_street]
        fig = px.bar(df_street, x='Street', y='price_per_m2_final', title=f"Biểu đồ giá/m2 theo Phường - Quận {selected_district} - TP {selected_city}")
        fig.update_layout(xaxis_title='Đường', yaxis_title='Trung vị giá / m2', xaxis_tickangle=-45, width=1200, height=700)
        st.plotly_chart(fig)
        ...
        # df_median3 = df_selected_city_districts.groupby(['City', 'District', 'Street'])['price_per_m2_final'].mean().reset_index()
        # # Tạo biểu đồ dựa trên DataFrame đã lọc
        # fig = px.bar(df_median3, x='Street', y='price_per_m2_final', title=f"Biểu đồ giá/m2 theo Đường - TP {selected_city}")
        ...
        # df_median3 = df_selected_city_streets.groupby(['City', 'Street'])['price_per_m2_final'].median().reset_index()
        # df_median3 = df_median3.sort_values(by='price_per_m2_final', ascending=False)
